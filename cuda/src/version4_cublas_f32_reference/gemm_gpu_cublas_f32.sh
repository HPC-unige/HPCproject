...
readonly CUDA_SOURCE_FILE="gemm_gpu_cublas_f32.cu"

readonly RESULTS_DIR="results_cuda/gemm_gpu_cublas_f32"
readonly EXECUTABLE_DIR="${RESULTS_DIR}/bin"
readonly RESULTS_CSV_PATH="${RESULTS_DIR}/gemm_gpu_cublas_f32.csv"
readonly LOG_PATH="${RESULTS_DIR}/gemm_gpu_cublas_f32.log"
...
readonly RUN_FINGERPRINT="SRC=${CUDA_SOURCE_FILE}|FLAGS=${COMMON_NVCC_FLAGS}|HEADER=${CSV_HEADER}|CASESET=cublas_f32_v1"
...
validate_metadata() {
    if [[ -f "$METADATA_PATH" ]]; then
        ...
        saved_fingerprint="$(cat "$METADATA_PATH")"

        if [[ "$saved_fingerprint" != "$RUN_FINGERPRINT" ]]; then
            log_message "ERROR: metadata mismatch in $METADATA_PATH"
            log_message "OLD: $saved_fingerprint"
...
prepare_csv_and_restore_state() {
    ...
    local dim_n runtime_s throughput_gflops
    while IFS=, read -r dim_n runtime_s throughput_gflops; do
        dim_n="$(strip_quotes "$dim_n")"

        [[ -z "$dim_n" ]] && continue
        [[ "$dim_n" == "ProblemSize" ]] && continue

        FINISHED_CASES["$dim_n"]=1
    done < "$RESULTS_CSV_PATH"
}

build_executable() {
    local dim_n="$1"
    local executable_path="${EXECUTABLE_DIR}/cublas_f32_N${dim_n}"

    echo "BUILD: N=$dim_n" | tee -a "$LOG_PATH" >&2
    ...
    if ! "$CUDA_COMPILER" $COMMON_NVCC_FLAGS \
        ...
        echo "BUILD FAILED: N=$dim_n" | tee -a "$LOG_PATH" >&2
        return 1
    fi
...
run_case() {
    ...
    if [[ -n "${FINISHED_CASES[$dim_n]:-}" ]]; then
        log_message "SKIP: already done | N=$dim_n"
        return 0
    fi

    log_message "RUN: N=$dim_n"

    local execution_output
    if ! execution_output="$("$executable_path" 2>&1)"; then
        log_message "RUN FAILED: N=$dim_n"
    ...
    if [[ -z "$runtime_s" || -z "$throughput_gflops" ]]; then
        log_message "PARSE FAILED: N=$dim_n"
        return 1
    fi
    ...
    log_message "DONE: N=$dim_n runtime=$runtime_s gflops=$throughput_gflops"
}
...
main() {
    ...
    for dim_n in "${MATRIX_DIMENSIONS[@]}"; do
        if ! executable_path="$(build_executable "$dim_n")"; then
            continue
        fi

        run_case "$dim_n" "$executable_path" || true
    done
