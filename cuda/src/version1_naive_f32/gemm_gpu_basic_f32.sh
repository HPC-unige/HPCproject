...
readonly CUDA_SOURCE_FILE="gemm_gpu_basic_f32.cu"

readonly RESULTS_DIR="results_cuda/gemm_gpu_basic_f32"
readonly EXECUTABLE_DIR="${RESULTS_DIR}/bin"
readonly RESULTS_CSV_PATH="${RESULTS_DIR}/gemm_gpu_basic_f32.csv"
readonly LOG_PATH="${RESULTS_DIR}/gemm_gpu_basic_f32.log"
readonly METADATA_PATH="${RESULTS_DIR}/run_meta.txt"

readonly CUDA_COMPILER="${NVCC:-nvcc}"
readonly CSV_HEADER="ProblemSize,TileShape,Runtime_s,Throughput_GFLOPS"
readonly COMMON_NVCC_FLAGS="-O3 -DALIGN -DRESTRICT"
readonly RUN_FINGERPRINT="SRC=${CUDA_SOURCE_FILE}|FLAGS=${COMMON_NVCC_FLAGS}|HEADER=${CSV_HEADER}|CASESET=naive_f32_v1"
...
prepare_csv_and_restore_state() {
    ...
    local dim_n tile_shape runtime_s gflops
    while IFS=, read -r dim_n tile_shape runtime_s gflops; do
        ...
        FINISHED_CASES["${dim_n}|${tile_shape}"]=1
...
build_executable() {
    ...
    local executable_path="${EXECUTABLE_DIR}/naive_f32_N${dim_n}_T${tile_w}x${tile_h}"

    echo "BUILD: N=$dim_n tile=$tile_shape" | tee -a "$LOG_PATH" >&2
    ...
    if ! "$CUDA_COMPILER" $COMMON_NVCC_FLAGS \
        ...
        echo "BUILD FAILED: N=$dim_n tile=$tile_shape" | tee -a "$LOG_PATH" >&2
        return 1
    fi
...
run_case() {
    ...
    local case_key="${dim_n}|${tile_shape}"
    if [[ -n "${FINISHED_CASES[$case_key]:-}" ]]; then
        log_message "SKIP: already done | N=$dim_n tile=$tile_shape"
        return 0
    fi

    log_message "RUN: N=$dim_n tile=$tile_shape"

    local execution_output
    if ! execution_output="$("$executable_path" 2>&1)"; then
        log_message "RUN FAILED: N=$dim_n tile=$tile_shape"
    ...
    if [[ -z "$runtime_s" || -z "$gflops" ]]; then
        log_message "PARSE FAILED: N=$dim_n tile=$tile_shape"
        return 1
    fi

    echo "$dim_n,$tile_shape,$runtime_s,$gflops" >> "$RESULTS_CSV_PATH"
    FINISHED_CASES["$case_key"]=1

    log_message "DONE: N=$dim_n tile=$tile_shape runtime=$runtime_s gflops=$gflops"
}
...
main() {
    ...
    for dim_n in "${MATRIX_DIMENSIONS[@]}"; do
        for tile_shape in "${TILE_CONFIGS[@]}"; do
            if ! executable_path="$(build_executable "$dim_n" "$tile_shape")"; then
                continue
            fi
            run_case "$dim_n" "$tile_shape" "$executable_path" || true
        done
    done
