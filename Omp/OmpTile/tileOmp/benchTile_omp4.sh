...
OUT_DIR="results_omp_tile"
BIN_DIR="${OUT_DIR}/bin"
ASM_DIR="${OUT_DIR}/asm"
LOG_DIR="${OUT_DIR}/logs"
CSV_FILE="${OUT_DIR}/omp_tile_report.csv"
...
# All three compilers included
COMPILERS=("icc" "icx")
OPT_LEVELS=("O3")

# Original sizes restored
SIZES=(6144 8192 10240 12288 )
...
# Standard OpenMP Affinity to prevent cache pollution
export OMP_PROC_BIND=true
export OMP_PLACES=cores

HEADER="CPU,Compiler,Opt,N,Tile,Threads,Schedule,Chunk,Align,Restrict,Time_Min_s,Time_Mean_s,GFLOPS_Min,GFLOPS_Mean,SIMD_Ops,FMA,AlignedMov,UnalignedMov,Status"
...
exists_done() {
  local cc="$1" opt="$2" n="$3" ts="$4" th="$5" sch="$6" chk="$7" al="$8" rs="$9"
  [[ ! -f "$CSV_FILE" ]] && return 1
  awk -F',' -v cc="$cc" -v opt="$opt" -v n="$n" -v ts="$ts" -v th="$th" \
             -v sch="$sch" -v chk="$chk" -v al="$al" -v rs="$rs" '
    $2==cc && $3==opt && $4==n && $5==ts && $6==th && $7==sch && $8==chk && $9==al && $10==rs {
      t=$11; st=$19;
...
compile_and_counts() {
  local cc="$1" opt="$2" n="$3" ts="$4" al="$5" rs="$6" exe="$7" asm="$8" log="$9"
  local flags="-std=c11 -${opt} $(omp_flag "$cc") $(arch_flags "$cc") -DN=${n} -DTILE_SIZE=${ts} -DENABLE_TIME -DVERIFY"
    ...
    for ts in "${TILE_SIZES[@]}"; do
      for th in "${THREAD_COUNTS[@]}"; do
        for sched_spec in "${SCHEDULE_SPECS[@]}"; do
          ...
          if exists_done "$cc" "O3" "$n" "$ts" "$th" "$sched" "$chunk" "$ALIGN_VALUE" "$RESTRICT_VALUE"; then
            echo "Skip: $cc N=$n Tile=$ts T=$th"
            continue
          fi
          ...
          tag="${cc}_N${n}_TS${ts}_T${th}_${sched}${chunk:+_${chunk}}"
          exe="${BIN_DIR}/${tag}"; asm="${ASM_DIR}/${tag}.s"; log="${LOG_DIR}/${tag}.log"
          echo "Running Tiled: $tag"

          counts=$(compile_and_counts "$cc" "O3" "$n" "$ts" "$ALIGN_VALUE" "$RESTRICT_VALUE" "$exe" "$asm" "$log")
          ...
          if [[ "$valid" -gt 0 ]]; then
            ...
            ref_key="N${n}_TS${ts}"; status="PASS"
            ...
            echo "${CPU_MODEL},${cc},O3,${n},${ts},${th},${sched},${chunk},1,1,${min_t},${mean_t},${gmin},${gmean},${counts},${status}" >> "$CSV_FILE"
          fi
        done
      done
    done
