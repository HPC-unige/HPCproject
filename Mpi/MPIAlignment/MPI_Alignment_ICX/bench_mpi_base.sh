...
# ===== ONLY INTEL MPI + mpiicx =====
MPICC_BIN="mpiicx"
MPI_WRAPPER="$(command -v "$MPICC_BIN" || true)"
if [[ -z "$MPI_WRAPPER" ]]; then
  echo "ERROR: mpiicx not found in PATH"
  exit 1
fi

if ! command -v icx >/dev/null 2>&1; then
  echo "ERROR: icx not found in PATH"
  echo "Run: source /opt/intel/oneapi/setvars.sh"
  exit 1
fi

# Prefer mpirun from the same Intel MPI bin directory
MPIRUN_BIN="${MPIRUN:-$(dirname "$MPI_WRAPPER")/mpirun}"
MPIRUN_EXTRA_ARGS="${MPIRUN_EXTRA_ARGS:-}"

if [[ ! -x "$MPIRUN_BIN" ]]; then
  echo "ERROR: Intel MPI mpirun not found at $MPIRUN_BIN"
  exit 1
fi

# Hardcode backend for this script
BACKEND_CC="icx"
BACKEND_BASE="icx"
...
if [[ ! -f "$ENV_FILE" ]]; then
  {
    ...
    echo
    echo "===== MPIICX ====="
    echo "MPI_WRAPPER=$MPI_WRAPPER"
    echo "ICX=$(command -v icx)"
    echo "SHOW=$("$MPICC_BIN" -show 2>&1 || true)"
    echo
    echo "===== MPIRUN ====="
...
# ===== fixed for mpiicx/icx only =====
build_compile_flags() {
  local opt="$1"
  local n="$2"
  echo "-std=c11 -${opt} -qopenmp-simd -xHost -DN=${n} -DTIME -DVERIFY -DALIGN -DRESTRICT -funroll-loops"
}

[[ -f "$SOURCE" ]] || { echo "ERROR: missing $SOURCE"; exit 1; }

echo "=== MPI BASE LIGHT BENCH (mpiicx only) ==="
echo "CPU         : $CPU_MODEL"
      ...
      for idx in "${!CORE_SET_NAMES[@]}"; do
        ...
        for ranks in 1 2 4 8 16 24; do
          ...
          if exists_done "$MPI_WRAPPER" "$BACKEND_CC" "$opt" "$n" "$ranks" "$bind" "$core_name" "$pin_csv"; then
            echo "SKIP | opt=$opt N=$n ranks=$ranks bind=$bind core_set=$core_name"
            continue
          fi
          ...
          echo
          echo "RUN  | opt=$opt N=$n ranks=$ranks bind=$bind core_set=$core_name"
          echo "PIN  | $pin_actual"
          echo "MEM  | root≈${ROOT_MEM_GB} GB worker≈${WORKER_MEM_GB} GB"

          LAST_COMPILE_FLAGS="$(build_compile_flags "$opt" "$n")"

          tmp_build_dir="$(mktemp -d "${TMPDIR:-/tmp}/mpi_base_XXXXXX")"
          exe="${tmp_build_dir}/app"
          tmp_cap="$(mktemp "${TMPDIR:-/tmp}/mpi_base_run_XXXXXX")"
          compile_log="${tmp_build_dir}/compile.log"

          compile_ok=1
          if ! "$MPICC_BIN" $LAST_COMPILE_FLAGS "$SOURCE" -o "$exe" >"$compile_log" 2>&1; then
            compile_ok=0
            echo "=== COMPILE FAILED ==="
            cat "$compile_log"
          fi
          ...
          echo "DONE | opt=$opt N=$n ranks=$ranks bind=$bind core_set=$core_name mean=${mean_t}s speedup=${speedup_mean:-NA} eff=${efficiency_pct:-NA}% status=$status"
        done
      done
    done
  done
...
echo
echo "Finished BASE_LIGHT (mpiicx only)"
echo "CSV : $CSV_FILE"
echo "REF : $REF_DB"
echo "ENV : $ENV_FILE"