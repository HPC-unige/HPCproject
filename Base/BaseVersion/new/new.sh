...
VEC_REGEX='\b(vmovapd|vmovupd|vmovaps|vmovups|vaddpd|vaddps|vmulpd|vmulps|vdivpd|vdivps|vfmadd[a-z0-9]*pd|vfmadd[a-z0-9]*ps)\b'

openmp_simd_flag() {
  local comp="$1"
  case "$comp" in
    gcc|icx) echo "-fopenmp-simd" ;;
    icc) echo "-qopenmp-simd" ;;
    *) echo "" ;;
  esac
}
...
run_one() {
  ...
  local omp_simd
  omp_simd="$(openmp_simd_flag "$comp")"

  $comp -std=c11 $flags $omp_simd -DVERIFY -DN=$n "$SOURCE_FILE" -o "$exe" -lm 2>/dev/null

  objdump -d "$exe" > "$asm"
  local simd
  simd=$(grep -cE "$VEC_REGEX" "$asm" 2>/dev/null || true)
  simd=${simd:-0}
